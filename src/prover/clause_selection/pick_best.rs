...
pub fn pick_best_clause(
    clauses: &mut HashMap<u64, Clause>,
    clause_order: &mut BinaryHeap<ClauseWeight>,
) -> Option<Clause> {
...
pub fn choose_heuristic(
    heuristic_use_count: &[usize],
    current_heuristic_count: &mut usize,
) -> usize {
    ...
    assert_eq!(
        *current_heuristic_count,
        heuristic_use_count.iter().fold(0, |acc, x| acc + x)
    );
    *current_heuristic_count = 1;
    0
}
